...
public sealed class DecksMappingProfile : Profile
{
    public DecksMappingProfile()
    {
        ...
        CreateMap<DeckCard, DeckCardItemResponse>()
            ...
            .ForCtorParam(nameof(DeckCardItemResponse.CardId), opt => opt.MapFrom(dc => dc.CardPrinting == null ? 0 : dc.CardPrinting.CardId))
            ...
            .ForCtorParam(nameof(DeckCardItemResponse.Game),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null && dc.CardPrinting.Card != null
                        ? dc.CardPrinting.Card.Game
                        : string.Empty))

            .ForCtorParam(nameof(DeckCardItemResponse.Set),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null
                        ? dc.CardPrinting.Set
                        : string.Empty))

            .ForCtorParam(nameof(DeckCardItemResponse.Number),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null
                        ? dc.CardPrinting.Number
                        : string.Empty))

            .ForCtorParam(nameof(DeckCardItemResponse.Rarity),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null
                        ? dc.CardPrinting.Rarity
                        : string.Empty))

            .ForCtorParam(nameof(DeckCardItemResponse.Style),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null
                        ? dc.CardPrinting.Style
                        : string.Empty))

            .ForCtorParam(nameof(DeckCardItemResponse.ImageUrl),
                opt => opt.MapFrom(dc =>
                    dc.CardPrinting != null
