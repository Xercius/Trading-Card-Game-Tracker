...
public record MoveToCollectionDto(
    int CardPrintingId, 
    int Quantity, 
    bool UseProxy = false
);
public record BulkSetWishlistDto(
        int CardPrintingId,
        int QuantityWanted
    );
...
namespace api.Controllers
{
    ...
    public class WishlistController : ControllerBase
    {
        ...
        private async Task<IActionResult> UpsertCore(int userId, UpsertWishlistDto dto)
        {
        ...
        // PUT bulk set (wanted)
        private async Task<IActionResult> BulkSetCore(int userId, IEnumerable<BulkSetWishlistDto> items)
        {
        ...
        private async Task<IActionResult> MoveToCollectionCore(int userId, MoveToCollectionDto dto)
        {
            if (dto is null) return BadRequest();
            if (dto.CardPrintingId <= 0) return BadRequest("CardPrintingId required.");
        ...
        [HttpPut]
        public async Task<IActionResult> BulkSet(int userId, [FromBody] IEnumerable<BulkSetWishlistDto> items)
        {
            if (UserMismatch(userId)) return StatusCode(403, "User mismatch.");
        ...
        public async Task<IActionResult> UpsertForCurrent([FromBody] UpsertWishlistDto dto)
        {
            if (!TryResolveCurrentUserId(out var uid, out var err)) return err!;
        ...
        public async Task<IActionResult> BulkSetForCurrent([FromBody] IEnumerable<BulkSetWishlistDto> items)
        {
            if (!TryResolveCurrentUserId(out var uid, out var err)) return err!;
        ...
        public async Task<IActionResult> MoveToCollectionForCurrent([FromBody] MoveToCollectionDto dto)
        {
            if (!TryResolveCurrentUserId(out var uid, out var err)) return err!;
